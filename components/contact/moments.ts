...
/**
 * THE EIGHT SUBJECTS, AS EIGHT PHOTOGRAPHS.
 *
 * The client's list of things they are open to talking about is not set as a list. Every
 * one of the eight is a thing that has already happened in this community and been
 * photographed, so each subject is a nameplate set into the picture it belongs to and the
 * page is walked rather than read.
 *
 * `alt` is copied VERBATIM from public/media/stills.json, which is the vision audit's own
 * description of what is in the frame. That matters: the word on the arch is a SUBJECT the
 * community is open to talking about, and the alt says what the picture actually shows.
 *
 * `event` is the frame's PROVENANCE and it is printed on the plate under the subject. It is
 * where the photograph was actually taken, never what the subject beside it is called. The
 * collection for each id is Context/Media/_audit/tagged.json; the collections are dated in
 * Context/Media/README.md (Pranava Workshop 13 April 2025; Samskrithi Sadhana was a single
 * day, 29 June 2025). Same form as components/within/frames.ts, which is the site's caption
 * system. No venue and no city are stated, because the audit states none — this project has
 * already shipped a Samskrithi Sadhana photograph captioned as a Yoga Mandala gathering once.
 *
...
export type Moment = {
  ...
  id: string;
  alt: string;
  /** where the frame was taken — provenance, printed on the plate. `on` is
   *  dropped on phones, where a 58%-wide doorway has no room for a date. */
  event: { at: string; on: string };
  focal: [number, number];
...
const PRANAVA_13 = { at: 'Pranava Workshop', on: '13 April 2025' };
 ...
 */
export const LANE = ['near', 'far', 'near', 'far', 'near', 'far', 'far', 'near'] as const;
