...
export function Share() {
  ...
  return (
    <section className="wi-share" id={`wi-${p.index}`} aria-labelledby={`wi-h-${p.index}`}>
      <figure className="wi-share__fig wi-onDark" data-wi="fig">
        <span className="wi-fig__mask wi-share__mask" data-drift>
          <picture>
            <source type="image/avif" srcSet={`/media/posters/${f.id}.avif`} />
            <img
              className="wi-fig__img"
        ...
        <div className="wi-share__say" data-wi="up">
          <p className="wi-share__no" aria-hidden="true">
            {p.index}
          </p>
          <h2 className="wi-share__name" id={`wi-h-${p.index}`}>
            <span className="sr">{`${p.index}. `}</span>
            {p.name}
          </h2>
          {p.lines.map((line) => (
            <p className="wi-share__line" key={line}>
              {line}
            </p>
          ))}
          {p.listLead ? <p className="wi-share__lead">{p.listLead}</p> : null}
        </div>

        <Chips items={p.items} className="wi-chips--wide" />
        <WallLabel f={f} />
      </figure>

      {closing ? (
        <div className="wi-share__closeIn" data-wi="up">
          <p className="wi-share__close">{closing}</p>
        </div>
      ) : null}
    </section>
  );
