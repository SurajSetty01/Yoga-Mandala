...
function Aperture({ f, eager }: { f: Frame; eager?: boolean }) {
  return (
    <span className="wi-fig__mask" data-drift>
      <picture>
...
export function Chips({ items, className = '' }: { items: readonly string[]; className?: string }) {
  return (
    <ul className={`wi-chips ${className}`.trim()}>
      {items.map((item, n) => (
        <li
          className="wi-chip"
          data-wi="chip"
          ...
          key={item}
        >
          <span className="wi-chip__in">{item}</span>
        </li>
      ))}
    </ul>
  );
      ...
      {within.pillars.slice(0, 3).map((p, i) => {
        ...
        return (
          <section
            ...
            aria-labelledby={`wi-h-${p.index}`}
            key={p.index}
          >
            <div className="wi-p__in">
              <div className="wi-p__say" data-wi="up">
                <p className="wi-p__no" aria-hidden="true">
                  {p.index}
                </p>
                <h2 className="wi-p__name" id={`wi-h-${p.index}`}>
                  <span className="sr">{`${p.index}. `}</span>
                  {p.name}
                </h2>
                {first ? <p className="wi-p__first">{first}</p> : null}
                {rest.map((line) => (
                  <p className="wi-p__line" key={line}>
                    {line}
                  </p>
                ))}
                {p.listLead ? <p className="wi-p__lead">{p.listLead}</p> : null}
              </div>

              <figure className="wi-fig" data-wi="fig">
